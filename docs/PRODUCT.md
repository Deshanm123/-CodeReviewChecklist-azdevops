...
> As a reviewer, I want each finding identified as QA, Code, or BA and to see its unique ID, so I can refer to a specific item unambiguously.

> As the assigned developer, I want to close and reopen each finding independently while retaining its resolution-attempt count, so repeated review cycles remain visible.
...
1. A dynamic list of review findings. Each finding has:
        - a review type: `QA Reviews`, `Code Reviews`, or `BA Reviews` (required);
   - a stable unique identifier generated by the backend;
   ...
   - a resolution-attempt count, incremented whenever the item moves from open to closed.
2. An **Add finding** action, usable repeatedly, so reviewers can add items throughout the review rather than in one batch.
3. A checklist view, ordered with the most severe findings first, where each item can be closed or reopened independently.
4. A simple progress indicator (e.g. `3 of 7 closed`).
...
- **Closing or reopening an item** is restricted to the user who is the **current, logged-in user matching the PBI's Developer field**. Only that person sees the status actions; everyone else sees the list read-only.
...
Only the assigned Developer closes or reopens items. A reviewer cannot change a finding's status on the developer's behalf in the MVP.
...
- Add a finding (review type, task, severity, optional description).
- Dynamic/repeated adding of findings during review.
- View findings list, ordered by severity.
- Display a stable unique identifier for every finding.
- Close and reopen a finding, restricted to the user matching the Developer field.
- Count each open-to-closed transition as a resolution attempt.
